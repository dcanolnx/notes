...
// ARGS //
*********/
def beforeDate = "31-12-2016"    // Formato en valor numerico -> dia-mes-anyo
def repositories = ["docker-releases", "releases"]
def path = "/tmp/"
    ...
    if (repo.name in repositories){
        ...
        try {
            ...
            components.each { Component comp ->
                if ( (comp.version().toString() =~ ($/.*(\d(-(([a-z]|[A-Z])|([a-z]|[0-9]){7}$)|latest|null)|PR\d).*/$)) ){
                    ...
                    try {
                        ...
                        assets.each { Asset asset ->
                            ...
                            // assetLastUpdateSplitted -> Fecha de last updated espliteada
                            alus = "${asset.attributes().child("content").get("last_modified")}".split(' ')

                            // Obtenemos la fecha de upload del asset
                            assetLastUpdate = DateTime.parse("${alus[2]}-${monthToNumber(alus[1])}-${alus[5]}", DateTimeFormat.forPattern("dd-M-yyyy"))

                            // Compara con la anterior fecha registrada
                            if ( assetLastUpdate.isAfter(lastDate.toInstant()) ){
                                // Si la fecha actual es posterior a la registrada se actualiza
                                lastDate = assetLastUpdate
                            }
                        }
...
class componentJson {
    String group, name, version, lastUpdate, format
    List<String> assetsURL

    componentJson(group, name, version, lastUpdate, format, assetsURL){
        this.group = group
        this.name = name
    ...
    componentJson(name, version, lastUpdate, format, assetsURL){
        this.group = null
        this.name = name
        this.version = version
        this.lastUpdate = lastUpdate
