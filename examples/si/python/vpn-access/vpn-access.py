...
def get_ara_lastExecutions(aradb_host, aradb_user, aradb_password, aradb_database, users,ansible):
    ...
    try:
        ...
        for u in users:
            ...
            AND ( 
                (ara.playbooks.status= "completed")
                OR
                (ara.playbooks.status= "running" AND TIMESTAMPDIFF(HOUR, ara.hosts.updated, NOW()) < 1)
                OR
                (ara.playbooks.status = "failed" AND TIMESTAMPDIFF(MINUTE, ara.hosts.updated, NOW()) < 120)
            )
            AND ara.hosts.playbook_id=ara.playbooks.id 
            AND ara.playbooks.path LIKE '%STRATIO-UBUNTU20-ENS/site-v7.yml'
            LIMIT 1"""
            cursor.execute(query)
            rows = cursor.fetchall()
...
if __name__ == "__main__":
    ...
    with open(logging_file, 'a') as archivo:
        archivo.write(f"  \n")
        archivo.write(f" ««« Inicio de ejecucion de VPN-ACCESS.PY a las {hora_actual} »»»  \n")
    #READ CONFIG FILE
    ...
    execution_count =  0
    execution_limit = 25 # LIMITE DE EJECUCUCIONES EN PARALELO IMPORTANTE EVITAR SATURAR XENON!!!!
    numero_playbooks_ansible = contar_playbooks_ansible()
    with open(logging_file, 'a') as archivo:
        archivo.write(f"   » Número de playbooks de Ansible en ejecución antes de lanzar esta tanda de hardenning:   {numero_playbooks_ansible}\n")
    ...
    print(users_lastexecution)
    for u in users_lastexecution:
        print(execution_count + execution_limit + numero_playbooks_ansible)
        if (execution_count == execution_limit or numero_playbooks_ansible > 30):
        ...
        if(u[1] is None):
            ...
            if time_difference > datetime.timedelta(days=6): #LIMITE DE DIAS !!!!! IMPORTANTE!!!
    ...
    hora_actual = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(logging_file, 'a') as archivo:
        archivo.write(f"   » Número de playbooks de Ansible en ejecución despues de lanzar esta tanda de hardenning: {numero_playbooks_ansible}\n")
        archivo.write(f" ««« Fin de ejecucion de VPN-ACCESS.PY a las {hora_actual} »»»  \n")
        archivo.write(f"  \n")
        archivo.write(f"  \n")
